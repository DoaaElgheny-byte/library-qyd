...
export class AddEditCustomerComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.minDateG = this.dateFormatter.ToGregorianDateStruct(
      '1800-01-01',
      'YYYY-MM-DD'
    );
    this.minDateH = this.dateFormatter.ToGregorianDateStruct(
      '1214-01-01',
      'YYYY-MM-DD'
    );
  }
  ...
  initForm() {
    this.form = this.fb.group({
      ...
      type: [CustomerType.Employee, Validators.compose([Validators.required])],
  ...
  getPlayerById() {
    ...
    this._employeeManagementService.getEmplyeeDetails(this.id).subscribe({
      next: (next) => {
        ...
        debugger
        this.initForm();
        this.form.patchValue(this.clientDetails);
        this.mobileNumber = next.data.mobileNumber;
        // this.form.get('mobileNumber')?.setValue(next.data.mobileNumber);
        // this.form.get('countryCodeIso')?.setValue(next.data.countryCodeIso);
        // this.form.get('countryCode')?.setValue(next.data.countryCode);
        //this.cdr.detectChanges();
  ...
  Back() {
    this.router.navigate(['/agent/departments/customer-management']);
  }
